...
* **Boosting Models**:
    ...
    * **lightgbm (lgb)**: 빠른 속도와 대용량 데이터 처리에 강점.

### 2-4. 하이퍼파라미터 최적화
* **hyperopt**: 단순 랜덤/그리드 서치가 아닌, **베이지안 최적화**를 통해 "더 가능성 높은 파라미터"를 탐색.
...
| **floors** | 층수 | 1.5층, 2.5층 등 소수점 존재. ( `int` 변환 시 정보 손실 주의) |
...
1.  **분포 확인의 중요성**: 무작정 모델을 돌리기 전에 QQ-Plot 등으로 정규성을 확인해야함.
2.  **모델별 데이터 준비**: 모든 모델에 스케일링을 하는 게 정답은 아님. 부스팅 모델에는 원본 데이터가 더 적합할 수 있음.

### ⚠️ 아쉬운 점 및 개선할 점 (Improvements)
* **Floors 변수 왜곡**: 원본 코드에서 `floors`를 `int`로 변환(`astype(int)`)하여 1.5층, 2.5층의 정보가 손실됨. 실제 분석에서는 `float` 타입을 유지하는 것이 바람직함.
* **Date 변수 활용**: `date` 컬럼을 삭제했는데, '연도'와 '월'을 추출했다면 집값의 **계절성(Seasonality)**이나 상승 추세를 반영하여 성능을 더 높일 수 있었을 것임.
* **지하실 변수**: `sqft_basement`가 0인 데이터가 많으므로, **'지하실 유무(Binary)'** 변수를 추가했다면 모델이 패턴을 더 쉽게 찾았을 것임.