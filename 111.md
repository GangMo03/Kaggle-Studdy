...
1.  **분포 확인의 중요성**: 무작정 모델을 돌리기 전에 QQ-Plot 등으로 정규성을 확인해야 한다.
2.  **모델별 데이터 준비**: 모든 모델에 스케일링을 하는 게 정답은 아니다. 부스팅 모델에는 원본 데이터가 더 적합할 수 있다.
3.  **하이퍼파라미터 튜닝**: `GridSearch`는 확실하지만 느리고, `Hyperopt`는 효율적이다. 상황에 맞춰 써야 한다.
